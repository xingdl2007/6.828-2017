...
boot_alloc(uint32_t n)
{
	...
	if(n > 0) {
		n = ROUNDUP(n, PGSIZE);
		// wrap around, we are out of memory
...
mem_init(void)
{
	...
	// LAB 3: Your code here.
	boot_map_region(kern_pgdir, UENVS, ROUNDUP(m, PGSIZE), PADDR(envs), PTE_U | PTE_P);
...
mem_init_mp(void)
{
	...
	for(int i = 0; i < NCPU; ++i) {
		boot_map_region(kern_pgdir, KSTACKTOP-(KSTKSIZE+KSTKGAP)*i-KSTKSIZE,
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	pte_t *pte = NULL;
	int cnt = size / PGSIZE;
	for(int i = 0; i < cnt; i++, va += PGSIZE, pa += PGSIZE) {
...
{
	// LAB 3: Your code here.
	const void *eva = va + len;
	uintptr_t addr = (uintptr_t)va;
	uintptr_t last = ROUNDUP((uintptr_t)eva, PGSIZE);

	while(addr < last) {
		if(addr >= ULIM ||
		   ( env->env_pgdir[PDX(addr)] & perm) != perm) {
